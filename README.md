...
- `OTEL_EXPORTER_OTLP_LOGS_ENDPOINT`: When set the server will push its logs to an external OLTP server. When not set other OLTP variables will be ignored. These sets the URL of the server to push the logs to.
- `OTEL_EXPORTER_OTLP_LOGS_PROTOCOL`: The protocol being used for the export. For example `http/protobuf`.
- `OTEL_EXPORTER_OTLP_LOGS_HEADERS`: Can contain key-value-pairs of header being also transmitted. Can be used for example for authentication.