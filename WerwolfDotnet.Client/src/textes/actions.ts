...
export const actionNames: Readonly<Record<ActionType, string>> = {
    ...
    [ActionType.SeerSelection]: "Seherin erwacht",
    [ActionType.SeerApprenticeSelection]: "Seherlehrling erwacht",
...
export const actionDescriptions: Readonly<Record<ActionType, string>> = {
    ...
    [ActionType.SeerApprenticeSelection]: "Der Seher ist verstorben und du bist nun der nächste Seher. Wähle einen Spieler, dessen Rolle du sehen möchtest.",
    [ActionType.WitchHealSelection]: "Wähle eine Person, die du heilen möchtest. Dies ist nur einmal pro Spiel möglich und du musst nicht zwingend heilen (Sollte niemand auswählbar sein, muss niemand geheilt werden).",
...
export const actionCompletions: Partial<Record<ActionType, (args: string[]) => string>> = {
    ...
    [ActionType.SeerApprenticeSelection]: args => `Du hast dich für <b>${args[0]}</b> entschieden. Dieser Spieler hat die Rolle <b>${roleNames[Role[args[1] as keyof typeof Role]]}</b>.`,
}